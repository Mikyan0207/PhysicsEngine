...
class [[nodiscard]] alignas(PE_VECTOR2_ALIGNMENT) Mat2x2
{
  public:
	...
	PE_INLINE float Mat2x2::operator()(std::uint32_t column, std::uint32_t row) const
	{
		PE_ASSERT(row < 2 && column < 2);
		return m_Cols[column].m_Value32[row];
	}

	PE_INLINE float& Mat2x2::operator()(std::uint32_t column, std::uint32_t row)
	{
		PE_ASSERT(row < 2 && column < 2);
		return m_Cols[column].m_Value32[row];
	}

	PE_INLINE Mat2x2 Mat2x2::operator*(Mat2x2 rhs) const;
	PE_INLINE Mat2x2 Mat2x2::operator+(Mat2x2 rhs) const;
	PE_INLINE Mat2x2 Mat2x2::operator-(Mat2x2 rhs) const;

	PE_INLINE Mat2x2 Mat2x2::operator*(float s) const;
	PE_INLINE Mat2x2 Mat2x2::operator+(float s) const;
	PE_INLINE Mat2x2 Mat2x2::operator-(float s) const;

	PE_INLINE Mat2x2& Mat2x2::operator*=(Mat2x2 rhs);
	PE_INLINE Mat2x2& Mat2x2::operator+=(Mat2x2 rhs);
	PE_INLINE Mat2x2& Mat2x2::operator-=(Mat2x2 rhs);

	PE_INLINE Mat2x2& Mat2x2::operator*=(float s);
	PE_INLINE Mat2x2& Mat2x2::operator+=(float s);
	PE_INLINE Mat2x2& Mat2x2::operator-=(float s);
